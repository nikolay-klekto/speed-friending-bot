...
import by.sf.bot.jooq.keys.RANDOM_COFFEE__RANDOM_COFFEE_USER_ID_FKEY
...
open class RandomCoffee(
    ...
    aliased,
    parameters,
    DSL.comment("Таблица для хранения информации об участниках Random Coffee"),
    TableOptions.table()
) {
    companion object {
    ...
    /**
     * The column <code>public.random_coffee.id_note</code>. Уникальный
     * идентификатор записи Random Coffee
     */
    val ID_NOTE: TableField<RandomCoffeeRecord, Int?> = createField(DSL.name("id_note"), SQLDataType.INTEGER.nullable(false).identity(true), this, "Уникальный идентификатор записи Random Coffee")

    /**
     * The column <code>public.random_coffee.user_id</code>. Идентификатор
     * пользователя
     */
    val USER_ID: TableField<RandomCoffeeRecord, Int?> = createField(DSL.name("user_id"), SQLDataType.INTEGER, this, "Идентификатор пользователя")

    /**
     * The column <code>public.random_coffee.interests</code>. Интересы
     * пользователя
     */
    val INTERESTS: TableField<RandomCoffeeRecord, String?> = createField(DSL.name("interests"), SQLDataType.VARCHAR, this, "Интересы пользователя")

    /**
     * The column <code>public.random_coffee.date_created</code>. Дата создания
     * записи
     */
    val DATE_CREATED: TableField<RandomCoffeeRecord, LocalDate?> = createField(DSL.name("date_created"), SQLDataType.LOCALDATE, this, "Дата создания записи")
    ...
    override fun getReferences(): List<ForeignKey<RandomCoffeeRecord, *>> = listOf(RANDOM_COFFEE__RANDOM_COFFEE_USER_ID_FKEY)
    ...
    fun users(): Users {
        if (!this::_users.isInitialized)
            _users = Users(this, RANDOM_COFFEE__RANDOM_COFFEE_USER_ID_FKEY)
